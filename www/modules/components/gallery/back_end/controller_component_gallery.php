...
class controller_component_gallery extends component {
	...
	public function action_edit() {
		...
		if(isset($_POST['priority']) && !empty($_POST['priority'])){
			$this->data['priority'] = $_POST['priority'];
		}
	...
	public function action_category() {
		if(file_exists(ROOT_DIR . '/modules/helpers/helper_save_image.php')){
			require_once(ROOT_DIR . '/modules/helpers/helper_save_image.php');
			$this->helper_image = new  helper_save_image;
		}else{
			$this->load_helper('image');
		}
		...
		if(isset($_POST['submit']) || isset($_POST['submit_exit'])) {
			...
			$data['priority'] = !empty($_POST['priority'])?$_POST['priority']:(!empty($category['priority'])?$category['priority']:'');
			...
			if(is_uploaded_file($_FILES['images']['tmp_name'][0])) {
				...
				if (pathinfo($_FILES['images']['name'][0], PATHINFO_EXTENSION) == 'zip'){
					set_time_limit(3600);
					...
					foreach($_FILES['images']['name'] as $key=>&$value){
						if(/*count($this->errors) == 0 && */!empty($value)){
							
							$images_list[$key] = $this->helper_image->img_upload($value, $_FILES['images']['tmp_name'][$key], $this->component_config['item_full_width'], $upload_dir . '/', $mini_width, false, $mini_height, $this->component_config['item_full_height']);
							$data['image'] = $images_list[$key];
							$this->model->add_item($data);
						}
					}
    ...
    public function action_sort_images(){
		$this->page['theme']['file'] = 'ajax';
		if(!empty($_POST['id'])){
			$result = $this->model->sort_images($_POST['id']);
		}else{
			$result = '';
		}
