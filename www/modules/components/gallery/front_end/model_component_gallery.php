...
class model_component_gallery extends model {
	...
	public function get_categories($sub=0) {
		$sql = "SELECT c.*, COUNT(i.id) AS count_items FROM gallery_categories c LEFT JOIN gallery_items i ON i.category_id = c.id WHERE c.sub=".$sub." GROUP BY c.id ORDER BY c.priority DESC";
